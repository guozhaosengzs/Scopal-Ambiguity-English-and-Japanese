...
```
<br>
Contrary to the results shown so far, when it comes to the gestures by English speakers, changing context "all > not" to "not > all" would decrease the gesture duration by around 354 ms, with a rather good significance. 
...
```
<br>
With the Japanese speakers, the significant correlation in the model is on the "most/many > not", and when compared with baseline, the gesture duration is around 114 ms. 
